package org.usfirst.frc.team5990.robot.commands;

import org.usfirst.frc.team5990.robot.Robot;
import org.usfirst.frc.team5990.robot.RobotMap;
import org.usfirst.frc.team5990.robot.subsystems.DoubleStraps;

import edu.wpi.first.wpilibj.command.Command;

/**
 *
 *
 */
public class CollectorFlip extends Command {

	public CollectorFlip() {
		// Use requires() here to declare subsystem dependencies
		requires(Robot.collector);
		setTimeout(RobotMap.TIMEOUT_FLIP);
	}

	// Called just before this Command runs the first time
	protected void initialize() {
	}

	// Called repeatedly when this Command is scheduled to run
	protected void execute() {
		Robot.collector.setTopMotor(RobotMap.POWER_TOP_FLIP);
		Robot.collector.setBottomMotor(RobotMap.POWER_BOTTOM_FLIP);
	}

	// Make this return true when this Command no longer needs to run execute()
	protected boolean isFinished() {
		return isTimedOut();
	}

	// Called once after isFinished returns true
	protected void end() {
		Robot.collector.setTopMotor(0);
		Robot.collector.setBottomMotor(0);
	}

	// Called when another command which requires one or more of the same
	// subsystems is scheduled to run
	protected void interrupted() {
	}
}
